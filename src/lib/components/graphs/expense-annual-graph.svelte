...
<Graph title="Annual Expenses" details={graphData.details}>
	<VisXYContainer data={graphData.data} height="250px">
		<VisLine {x} {y} color="var(--red)" />
		<VisScatter {x} {y} color="var(--red)" />
		<VisAxis type="x" gridLine={false} minMaxTicksOnly={true} />
		<VisAxis type="y" label="Total Expense (£)" tickFormat={(y) => currencyFormat(y)} />
		<VisCrosshair {template} color="var(--red)" />
		<VisTooltip />
	</VisXYContainer>
</Graph>
